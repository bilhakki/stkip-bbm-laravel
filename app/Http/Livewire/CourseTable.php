...
class CourseTable extends DataTableComponent
{
    ...
    public function builder(): Builder
    {
        $courses = Course::query()
            ->with(['major', 'faculty']) // Eager load anything
        ...
        if ($role === UserRole::ACADEMIC_FACULTY) {
            ...
            $courses = $courses->whereHas('faculty', function ($query) use ($faculty_id) {
                $query->where('id', $faculty_id);
            });
        } else if ($role === UserRole::ACADEMIC_MAJOR) {
            $major_id = auth()->user()->academic->academicable_id;
            $courses = $courses->whereHas('major', function ($query) use ($major_id) {
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Credits", "credits")->sortable(),
            Column::make("Nama", "name")
                ...
                ->sortable()->searchable(),
            Column::make("Fakultas", "major.faculty.name")->sortable(),
            Column::make("Jurusan", "major.name")->sortable(),
