...
>
    ...
    <form
        ...
        method="post"
        action="{{ route('tuition-payment.store') }}"
        autocomplete="off"
    ...
    >
        ...
        <div class="space-y-2">
            <div class="grid grid-cols-1 gap-3">
                <div>
                    <x-label value="{{ __('Tanggal pembayaran') }}" />
                    <x-input
                        class="w-full"
                        type="datetime-local"
                        name="payment_at"
                        x-model="body.payment_at"
                    />
                </div>

                <div>
                    <x-label value="{{ __('Pilih mahasiswa') }}" />
                    <x-select
                        class="!px-3 !py-2"
                        data-url="{{ route('student.index') }}"
                        name="student_id"
                ...
                <div>
                    <x-label value="{{ __('Nomor kwitansi pembayaran') }}" />
                    <x-input
                        class="w-full"
                        type="text"
                        name="receipt_number"
                        x-model="body.receipt_number"
                    />
                </div>

                <div>
                    <x-label value="{{ __('Status') }}" />
                    <x-select x-model="body.status">
                        <option hidden>Status</option>
                        @foreach (TuitionPaymentStatus::values() as $item)
                            <option
                                class="uppercase"
                                value="{{ $item }}"
                ...
                <div>
                    <x-label value="{{ __('Pilih semester') }}" />
                    <x-select
                        class="!px-3 !py-2"
                        data-url="{{ route('season.index') }}"
                        name="season_id"
                        x-model="body.season_id"
                    >
